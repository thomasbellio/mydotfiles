...
Profiles are simple zsh files that call the 'register_module' function defined in the [module-manager](../module-manager.zsh).
...
Profiles can be stacked, so you might include the [default](./default.zsh) profile along with the [android](./android.zsh) profile. Profiles are loaded in an idempotent manner, so each profile may define the same modules, but they will only each be registered and sourced once.  

The primary way to configure a module is to use the `DOTFILES_ZSH_MODULE_PROFILES` environment variable. Which can be loaded in your `.zprofile` or any other place that will be sourced before the `.zshrc` file. 
